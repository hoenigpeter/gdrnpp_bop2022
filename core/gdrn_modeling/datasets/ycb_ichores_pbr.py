...
class YCB_ICHORES_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.ycb_ichores.id2obj.items() if obj_name in self.objs]
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(
                    self.models_root,
                    f"obj_{ref.ycb_ichores.obj2id[obj_name]:06d}.ply",
                ),
...
def get_ycb_ichores_metadata(obj_names, ref_key):
...
ycb_ichores_model_root = "BOP_DATASETS/ycb_ichores/models/"
################################################################################


SPLITS_YCB_ICHORES  _PBR = dict(
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_YCB_ICHORES_PBR:
        ...
        used_cfg = data_cfg
    DatasetCatalog.register(name, YCB_ICHORES_PBR_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="ycb_ichores",  # NOTE: for pvnet to determine module
        ref_key=used_cfg["ref_key"],
        objs=used_cfg["objs"],
        eval_error_types=["ad", "rete", "proj"],
        evaluator_type="bop",
        **get_ycb_ichores_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )
