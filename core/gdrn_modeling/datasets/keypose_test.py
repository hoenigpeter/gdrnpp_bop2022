...
class KEYPOSE_BOP_TEST_Dataset(object):
    """keypose bop test."""
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg["dataset_root"]  # BOP_DATASETS/keypose/test_primesense
...
#     name = "keypose_{}_bop_test_primesense".format(obj)
#     select_objs = [obj]
#     if name not in SPLITS_KEYPOSE:
#         SPLITS_KEYPOSE[name] = dict(
#             name=name,
#             dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/keypose/test_primesense"),
#             models_root=ref.keypose.model_dir,
#             objs=[obj],
#             ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/keypose/test_targets_bop19.json"),
#             scale_to_meter=0.001,
#             with_masks=True,  # (load masks but may not use it)
#             with_depth=True,  # (load depth path here, but may not use it)
#             height=540,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_KEYPOSE:
        used_cfg = SPLITS_KEYPOSE[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, KEYPOSE_BOP_TEST_Dataset(used_cfg))
...
def get_available_datasets():
    return list(SPLITS_KEYPOSE.keys())
