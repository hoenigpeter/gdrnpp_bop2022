...
class KEYPOSE_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get("dataset_root", osp.join(DATASETS_ROOT, "BOP_DATASETS/keypose_transparent/train_pbr"))
        ...
        self.models_root = data_cfg["models_root"]  # BOP_DATASETS/keypose/models_cad
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.keypose_transparent.id2obj.items() if obj_name in self.objs]
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(self.models_root, f"obj_{ref.keypose_transparent.obj2id[obj_name]:06d}.ply"),
...
def get_keypose_metadata(obj_names, ref_key):
...
keypose_model_root = "BOP_DATASETS/keypose/models/"
################################################################################


SPLITS_KEYPOSE_PBR = dict(
    keypose_transparent_train_pbr=dict(
        name="keypose_transparent_train_pbr",
        objs=ref.keypose_transparent.objects,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/keypose_transparent/train_pbr"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/keypose_transparent/models"),
        xyz_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/keypose_transparent/train_pbr/xyz_crop"),
...
for obj in ref.keypose_transparent.objects:
    for split in ["train_pbr"]:
        name = "keypose_transparent_{}_{}".format(obj, split)
...
#     for split in ["train_pbr"]:
#         name = "keypose_{}_{}".format(obj, split)
#         if name not in SPLITS_KEYPOSE_PBR:
#             SPLITS_KEYPOSE_PBR[name] = dict(
#                 name=name,
#                 objs=[obj],  # only this obj
#                 dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/keypose/train_pbr"),
#                 models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/keypose/models_cad"),
#                 xyz_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/keypose/train_pbr/xyz_crop"),
#                 scale_to_meter=0.001,
#                 with_masks=True,  # (load masks but may not use it)
#                 with_depth=True,  # (load depth path here, but may not use it)
#                 height=720,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_KEYPOSE_PBR:
        used_cfg = SPLITS_KEYPOSE_PBR[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, KEYPOSE_PBR_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="keypose",  # NOTE: for pvnet to determine module
        ref_key=used_cfg["ref_key"],
        objs=used_cfg["objs"],
        eval_error_types=["ad", "rete", "proj"],
        evaluator_type="bop",
        **get_keypose_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )


def get_available_datasets():
    return list(SPLITS_KEYPOSE_PBR.keys())
